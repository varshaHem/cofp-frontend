...
import { debounceTime, distinctUntilChanged, filter, first, map, mergeAll, Observable, pluck, retry, startWith, switchMap, tap} from 'rxjs';
...
export class SearchComponent implements OnInit {
  ...
  constructor(private searchService: SearchService) {
    ...
    this.results$ = this.search.valueChanges.pipe(
      tap(x => console.log(x)),
      // map((x) => x.toLowerCase()),
      debounceTime(2000),  // user has to stop typing for 200ms for us to bother the server with that query
